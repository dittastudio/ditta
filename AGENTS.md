...
.storyblok/
  schema/                    # Code-first Storyblok schema — the source of truth, pushed to the space
public/
...
- `@/` → `app/`
- `~/` → project root
- `#storyblok-schema` → the code-first Storyblok schema and its inferred types
...
- **Block and story types are inferred from the schema, never hand-written.** Type a component's props with `Block<'block_name'>` from `#storyblok-schema`, keyed by the Storyblok block name: `block: Block<'block_accordion'>`. For a whole story, `ISbStoryData<Block<'page'>>`.
- Field value types (`RichTextFieldValue`, `AssetFieldValue`, `MultilinkFieldValue`) are imported from `@storyblok/schema` directly.
- **Optional fields are `T | null | undefined`.** The CDN sends `null` for an unset field, so a prop that receives one declares `| null` rather than coercing at the call site.
...
- Multilinks (`MultilinkFieldValue`) should be rendered through the existing `Link.vue` component, which normalizes `story` / `email` / `url` link types and wraps `NuxtLink`.
...
- Relation fields (`options` with `source: 'internal_stories'`) are typed as UUID strings, because that is what the API returns without `resolve_relations`. Read them through `storyblokRelations<'project'>(block.projects)`, which widens to the resolved stories and drops anything left unresolved.

### Changing the schema

The schema lives in `.storyblok/schema/` and is **code-first**: edit the block definitions there, then push. Editing a block in the Storyblok UI will drift from these files and will not reach the app's types.
...
npm run sb:schema-push       # push local schema to space 284609
npm run sb:schema-rollback   # restore the space from a changeset
```

**Always run `sb:schema-diff` before `sb:schema-push`.** A push writes to the live space, and the local schema is not a full mirror of it: `push` only creates and updates, so a block deleted locally survives remotely until someone passes `--delete`.

Each push writes a changeset to `.storyblok/schema/changesets/` recording the full pre-push remote state. `sb:schema-rollback` replays one to restore the space, so **commit changesets** — without them there is no undo. It prompts for which changeset to use; add `--latest` to take the most recent without prompting.

`sb:schema-push` passes `--no-write-components`, because the block definitions in `.storyblok/schema/` are already the source of truth and a parallel JSON snapshot would only drift from them.

`sb:schema-init` is a one-time bootstrap that **overwrites** `.storyblok/schema/`. It has already been run; do not run it again without coordination. The 284609 space ID is wired into these scripts — don't change it without coordination.

### Content migrations

**Schema and content are migrated separately.** A schema push changes the _shape_ of a block; it does not touch content already saved in existing stories. Rename a field from `copy` to `text` and every published story still holds its text under `copy`, while the new field reads empty. A content migration is what moves it.
...
npm run sb:migrations-rollback -- <file>       # restore content from the pre-run snapshot
```

Note the `--`: the component name is an argument to the CLI, not to npm.

Migrations live in `.storyblok/migrations/284609/` and are ordinary JS modules that take a block's content and return it transformed — commit them, they are code. Each run first writes a content snapshot to `.storyblok/migrations/284609/rollbacks/`, which is what `sb:migrations-rollback` replays.

When `sb:schema-push` detects a breaking change it offers to scaffold the matching migration files (`--no-migrations` to decline). **Those scaffolds are starting points, not finished migrations** — read and adjust each one before running it.

`sb:migrations-run` saves stories without publishing, so changes land as drafts. Pass `--publish published` to republish stories that were already live, or `--publish all` for everything. Scope a run with `--starts-with="/work/"` or `--query="[highlighted][in]=true"` when a migration should not touch the whole space.
...
| Change a Storyblok block        | Edit `.storyblok/schema/`, then `npm run sb:schema-push`             |
...
Issues and PRDs are tracked as GitHub issues in `dittastudio/ditta` via the `gh` CLI. See `docs/agents/issue-tracker.md`.
